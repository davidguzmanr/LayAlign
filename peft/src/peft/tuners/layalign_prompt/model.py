...
class AdaptionPromptModel(nn.Module):
    ...
    def __init__(self, model, configs: Dict, adapter_name: str):
        ...
        #     configs[adapter_name].adapter_batch, configs[adapter_name].num_encoder_layers, 1, configs[adapter_name].encoder_hidden_dim)
    ...
    def add_adapter(self, adapter_name: str, config: AdaptionPromptConfig) -> None:
        ...
        for name, _ in self.model.named_modules():
            if name.endswith(config.target_modules):
                ...
                parents.append(par)
        if len(parents) < len(config.adapter_layers):
            raise ValueError(
        ...
        #####L 层
        parents_layer = []
        for adapter_layer in config.adapter_layers:
            parents_layer.append(parents[adapter_layer])
        parents = parents_layer
